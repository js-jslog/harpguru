...
import { putActiveOptionThird } from './put-active-option-third'
...
test('complete list of PitchIds with the active id at the end is reordered appropriately', () => {
  ...
  const expectedResult = getPitchIds(PitchIds.A)
  const actualResult = putActiveOptionThird(orderedOptionIds, activeOptionId)
...
test('complete list of PitchIds with the active id in the middle is reordered appropriately', () => {
  ...
  const expectedResult = getPitchIds(PitchIds.D)
  const actualResult = putActiveOptionThird(orderedOptionIds, activeOptionId)
