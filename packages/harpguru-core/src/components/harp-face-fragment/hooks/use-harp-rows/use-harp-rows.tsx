...
export const useHarpRows = (xRange: XRange, harpfaceIndex: 0 | 1): HarpRows => {
  ...
  const fullBlowDrawIdsMap = fullInteractionMatrix[harpfaceIndex].map(mapRowToBlowDrawIds)
  ...
  const topHarpRows = topRowsPrimer.map((_, index) => {
    return (
      <HarpRow key={index} xRange={xRange} yCoord={index + drawIndexDiff} harpfaceIndex={harpfaceIndex} />
    )
  })
  const bottomHarpRows = bottomRowsPrimer.map((_, index) => {
    const amendedIndex = index + topHarpRows.length + drawIndexDiff
    return <HarpRow key={amendedIndex} xRange={xRange} yCoord={amendedIndex} harpfaceIndex={harpfaceIndex} />
  })
