...
export const buildApparatus = (
  ...
  const halfstepIndexMatrix = [reedArrayToMatrices(reedArray1, tuningId, valvingId).halfstepIndexMatrix, (isReedArrayEmpty(reedArray2) ? [] : reedArrayToMatrices(reedArray2, tuningId, valvingId).halfstepIndexMatrix)] as const
  const  interactionMatrix = [reedArrayToMatrices(reedArray1, tuningId, valvingId).interactionMatrix, (isReedArrayEmpty(reedArray2) ? [] : reedArrayToMatrices(reedArray2, tuningId, valvingId).interactionMatrix)] as const
  return {
    tuningId,
    valvingId,
    halfstepIndexMatrix,
