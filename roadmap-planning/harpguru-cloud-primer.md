...
_Drafted in `js-jslog/harpguru` at `roadmap-planning/harpguru-cloud-primer.md`. Copy it into
`harpguru-cloud` as `docs/orientation.md` when the fork is made, and delete this line. It is
seeded from here once; after that the copy in `harpguru-cloud` is the live one and this file
can go._
